...
func writeGoogetRepos(repos []*osconfigpb.GooRepository, repoFile string) error {
	/*
...
func updateGoogetPackages(packageInstalls, packageRemovals []*osconfigpb.Package) error {
	var errs []string
	...
	if changes.packagesToInstall != nil {
		...
		if err := packages.InstallGooGetPackages(changes.packagesToInstall); err != nil {
			errStr := fmt.Sprintf("Error installing googet packages: %v", err)
	...
	if changes.packagesToUpgrade != nil {
		...
		if err := packages.InstallGooGetPackages(changes.packagesToUpgrade); err != nil {
			errStr := fmt.Sprintf("Error upgrading googet packages: %v", err)
			logger.Errorf(errStr)
			errs = append(errs, errStr)
		}
	}
	...
	if changes.packagesToRemove != nil {
		...
		if err := packages.RemoveGooGetPackages(changes.packagesToRemove); err != nil {
			errStr := fmt.Sprintf("Error removing googet packages: %v", err)
			logger.Errorf(errStr)
			errs = append(errs, errStr)
		}
	}
