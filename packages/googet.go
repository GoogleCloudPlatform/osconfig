...
func parseGooGetUpdates(data []byte) []*PkgInfo {
	...
	*/
	...
	for _, ln := range lines {
		...
		pkgs = append(pkgs, &PkgInfo{Name: p[0], Arch: strings.Trim(p[1], ","), Version: pkg[3], Type: typeGooGet})
	}
	return pkgs
}
...
func parseInstalledGooGetPackages(data []byte) []*PkgInfo {
	...
	*/
	...
	for _, ln := range lines {
		...
		pkgs = append(pkgs, &PkgInfo{Name: string(p[0]), Arch: string(p[1]), Version: string(pkg[1]), Type: typeGooGet})
	}
	return pkgs
}
